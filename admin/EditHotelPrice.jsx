...
const EditHotelAvailabilityAndPrices = () => {
  ...
  // Transform API prices data to our new structure (preserve purchase_price and compute profit)
  const transformPrices = (apiPrices) => {
    ...
    apiPrices.forEach(price => {
      ...
      if (price.room_type === "Only-Room") {
        groupedPrices[key].only_room_price = price.price ?? groupedPrices[key].only_room_price;
        groupedPrices[key].only_room_purchase_price = price.purchase_price ?? groupedPrices[key].only_room_purchase_price;
      }
  ...
  // Handle bed purchase price change
  const handleBedPurchaseChange = (sectionId, bedType, value) => {
    setHotelData(prevData => ({
      ...prevData,
      prices: prevData.prices.map(section => {
        if (section.id === sectionId) {
          const updatedBedPrices = section.bed_prices.map(bp =>
            bp.type === bedType ? { ...bp, purchase_price: value } : bp
          );
  ...
  // Handle only room purchase price change
  const handleOnlyRoomPurchaseChange = (sectionId, value) => {
    setHotelData(prevData => ({
      ...prevData,
      prices: prevData.prices.map(section =>
        section.id === sectionId ? { ...section, only_room_purchase_price: value } : section
      ),
    }));
  };
  ...
  // Prepare data for API submission (include purchase_price and profit)
  const preparePriceDataForAPI = () => {
    const allPriceEntries = [];

    hotelData.prices.forEach((section) => {
      // Add the Only-Room price entry
      const onlyPrice = parseFloat(section.only_room_price) || 0;
      const onlyPurchase = parseFloat(section.only_room_purchase_price) || 0;
      const onlyProfit = Number((onlyPrice - onlyPurchase).toFixed(2));
      allPriceEntries.push({
        start_date: section.start_date,
        end_date: section.end_date,
        room_type: "Only-Room",
        price: onlyPrice,
        purchase_price: onlyPurchase,
        profit: onlyProfit,
      ...
      section.bed_prices.forEach((bedPrice) => {
        ...
        const priceVal = parseFloat(bedPrice.price) || 0;
        const purchaseVal = parseFloat(bedPrice.purchase_price) || 0;
        const profitVal = Number((priceVal - purchaseVal).toFixed(2));
                  ...
                  <div key={section.id} className="p-4 mb-3 position-relative">
                    ...
                    <div className="row">
                      <div className="col-12 d-flex flex-wrap gap-4">
                        ...
                        {/* Only-Room purchase/cost and profit */}
                        <div>
                          <label htmlFor="" className="">
                            Only-Room Cost
                          </label>
                          <input
                            type="number"
                            value={section.only_room_purchase_price || ""}
                            onChange={(e) =>
                              handleOnlyRoomPurchaseChange(section.id, e.target.value)
                            }
                            className="form-control "
                            placeholder="SAR 20"
                          />
                          ...
                          <div className="mt-2">
                            <small className="text-muted">
                              Profit: SAR {Number((parseFloat(section.only_room_price || 0) - parseFloat(section.only_room_purchase_price || 0)).toFixed(2))}
                            </small>
                          </div>
                        </div>
                          ...
                          return (
                            <label key={bedPrice.type} className="rounded position-relative" style={{ width: "200px" }}>
                              ...
                              <input
                                ...
                                placeholder="SAR 30"
                              />
                              {/* Purchase price for this bed type */}
                              <input
                                type="number"
                                value={bedPrice.purchase_price || ""}
                                onChange={(e) =>
                                  handleBedPurchaseChange(
                                    section.id,
                                    bedPrice.type,
                                    e.target.value
                                  )
                              ...
                              <div className="mt-2">
                                <small className="text-muted">
                                  Profit: SAR {Number((parseFloat(bedPrice.price || 0) - parseFloat(bedPrice.purchase_price || 0)).toFixed(2))}
                                </small>
                              </div>
                            </label>
                          );
