...
const BlogManagement = () => {
  ...
  // Use hook to fetch blogs from backend. See src/hooks/useBlogs.jsx
  // data.results will be the array of blog objects returned by the API
  const { data, loading, error, params, setParams, refresh } = useBlogs();

  // Map API blog shape to the UI shape used across this component
  const blogs = (data?.results || []).map(b => ({
    id: b.id,
    title: b.title,
    slug: b.slug,
    short_description: b.summary || '',
    cover_image: b.cover_image || '',
    author_id: b.author_id || b.author || 1,
    author_name: b.author_name || (b.author && b.author.username) || 'Admin',
    status: b.status || 'draft',
    tags: (b.meta && Array.isArray(b.meta.tags)) ? b.meta.tags : [],
    hashtags: (b.meta && Array.isArray(b.meta.hashtags)) ? b.meta.hashtags : [],
    meta_title: b.meta?.meta_title || b.meta?.title || '',
    meta_description: b.meta?.meta_description || b.meta?.description || '',
  views: b.views || 0,
  // prefer likes_count from API (includes anonymous likes), fall back to legacy 'likes'
  likes: (typeof b.likes_count !== 'undefined' ? b.likes_count : (b.likes || (b.meta && b.meta.anonymous_likes) || 0)),
    comments: b.comments_count || 0,
    created_at: b.published_at || b.created_at || '',
    updated_at: b.updated_at || '',
    page_url: `/blogs/${b.slug}/`,
    sections: b.sections || [],
  }));
  ...
  const handleSaveBlog = async () => {
    ...
    const metaObj = {
      ...
      hashtags: blogForm.hashtags ? blogForm.hashtags.split(',').map(h => h.replace(/^#/, '').trim()).filter(Boolean) : [],
      meta_title: blogForm.meta_title,
      meta_description: blogForm.meta_description,
    };

    let payloadToSend;
    // If cover_image is a File, send multipart/form-data
    if (blogForm.cover_image && typeof blogForm.cover_image === 'object' && blogForm.cover_image instanceof File) {
      const fd = new FormData();
      fd.append('title', blogForm.title);
      fd.append('slug', slug);
      fd.append('summary', blogForm.short_description);
      fd.append('status', selectedBlog ? selectedBlog.status : 'draft');
      fd.append('cover_image', blogForm.cover_image);
      // JSONField should be sent as JSON string in multipart
      fd.append('meta', JSON.stringify(metaObj));
      payloadToSend = fd;
    } else {
      // For JSON requests we omit cover_image (so existing image remains unless a new file is uploaded)
      payloadToSend = {
        title: blogForm.title,
        slug,
        summary: blogForm.short_description,
        meta: metaObj,
        status: selectedBlog ? selectedBlog.status : 'draft',
      };
    }

    try {
      if (selectedBlog) {
        await blogApi.patchBlog(selectedBlog.id, payloadToSend);
        showAlert('success', `Blog updated successfully. Page URL: ${generatePageUrl(slug)}`);
      } else {
        await blogApi.createBlog(payloadToSend);
        showAlert('success', `Blog created successfully. Auto page created: ${generatePageUrl(slug)}`);
      }
      await refresh();
    } catch (err) {
      showAlert('danger', err?.response?.data?.detail || err.message || 'Failed to save blog');
    } finally {
  ...
  const handleDeleteBlog = async (id) => {
    if (!window.confirm('Are you sure you want to delete this blog? The associated page will be marked inactive.')) return;
    try {
      await blogApi.deleteBlog(id);
      showAlert('success', 'Blog deleted and page marked inactive');
      await refresh();
    } catch (err) {
      showAlert('danger', err?.response?.data?.detail || err.message || 'Failed to delete blog');
    }
  };
  ...
  const handleChangeStatus = async (id, newStatus) => {
    try {
      await blogApi.patchBlog(id, { status: newStatus });
      showAlert('success', `Blog status changed to ${newStatus}`);
      await refresh();
    } catch (err) {
      showAlert('danger', err?.response?.data?.detail || err.message || 'Failed to change status');
    }
  };
                          ...
                          <td className="tags-col">
                            <div className="tags-wrap">
                              {blog.tags.slice(0, 2).map(tag => (
                                <Badge key={tag} bg="info">{tag}</Badge>
                              ))}
          ...
          <Modal show={showAddModal} onHide={() => setShowAddModal(false)} size="lg">
            ...
            <Modal.Body>
              <Form>
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Cover Image (upload)</Form.Label>
                  <Form.Control
                    type="file"
                    accept="image/*"
                    onChange={(e) => setBlogForm({ ...blogForm, cover_image: e.target.files && e.target.files[0] ? e.target.files[0] : null })}
                  />
                  <small className="text-muted d-block mt-1">Upload an image which will be used on cards and the blog page. If you leave this empty, the existing image (if any) will remain.</small>
                </Form.Group>
              </Form>
            </Modal.Body>
            <Modal.Footer>
