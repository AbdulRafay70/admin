...
const AddHotels = () => {
  ...
  useEffect(() => {
    const fetchCities = async () => {
      ...
      try {
        ...
        // toast may be undefined in this file; keep defensive logging only
      } finally {
  ...
  // Handle bed purchase price change (how much we purchase per night)
  const handleBedPurchasePriceChange = (sectionId, bedType, value) => {
    setPriceSections(prevSections =>
      prevSections.map(section => {
        if (section.id === sectionId) {
          const updatedBedPrices = section.bed_prices.map(bp =>
  ...
  const handleOnlyRoomPurchasePriceChange = (sectionId, value) => {
    setPriceSections(prevSections =>
      prevSections.map(section =>
    ...
    try {
      ...
      priceSections.forEach((section) => {
        ...
        allPriceEntries.push({
          ...
          purchase_price: parseFloat(section.only_room_purchase_price) || 0,
          profit: Number(((parseFloat(section.only_room_price) || 0) - (parseFloat(section.only_room_purchase_price) || 0)).toFixed(2)),
        ...
        section.bed_prices.forEach((bedPrice) => {
          ...
          allPriceEntries.push({
            ...
            purchase_price: parseFloat(bedPrice.purchase_price) || 0,
            profit: Number(((parseFloat(bedPrice.price) || 0) - (parseFloat(bedPrice.purchase_price) || 0)).toFixed(2)),
            is_sharing_allowed: isSharing
          });
        });
      });

      // Map frontend category values to backend choices
      ...
      const mappedCategory = categoryMapping[hotelDetails.category] || hotelDetails.category || "standard";

      // Prepare data according to API structure
      const data = {
        name: hotelDetails.name,
        city: Number(hotelDetails.city) || hotelDetails.city,
                      ...
                      <div className="col-12 d-flex flex-wrap gap-5">
                        ...
                        <div>
                          ...
                          <input
                            ...
                            className="form-control rounded shadow-none  px-1 py-2"
                            placeholder="Selling price (per night)"
                            required
                          />
                        </div>
                        <div>
                          <label htmlFor="" className="">
                            Purchase Price (per night)
                          </label>
                          <input
                            type="number"
                            value={section.only_room_purchase_price}
                            onChange={(e) =>
                              handleOnlyRoomPurchasePriceChange(section.id, e.target.value)
                            }
                            className="form-control rounded shadow-none  px-1 py-2"
                            placeholder="How much we purchase per night"
                          />
                        </div>
                        {/* Settling price removed */}
                        ...
                        {section.bed_prices.map((bedPrice) => {
                          ...
                          return (
                            <div key={bedPrice.type} className="rounded mb-3 position-relative d-flex flex-column">
                              <button
                                type="button"
                                className="btn btn-danger position-absolute"
                              ...
                              <div className="d-flex gap-2">
                                ...
                                <input
                                  ...
                                  className="form-control rounded shadow-none  px-1 py-2"
                                  placeholder="Purchase price per night"
                                />
                                {/* Settling price removed */}
                              </div>
                            </div>
