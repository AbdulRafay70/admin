...
export const AuthProvider = ({ children }) => {
  ...
  const adminLogin = async (access, refresh) => {
    try {
      ...
      // ensure axios instance sends the Authorization header immediately
      try {
        api.defaults.headers = api.defaults.headers || {};
        api.defaults.headers.Authorization = access.startsWith("Bearer ") ? access : `Bearer ${access}`;
      } catch (e) {
  ...
  const logout = () => {
    ...
    setUserRole(null);
    try {
      if (api && api.defaults && api.defaults.headers) delete api.defaults.headers.Authorization;
    } catch (e) {}
    navigate("/login");
  };
