...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Use central api wrapper so baseURL and auth handling is consistent
  ...
  // Check user profile type using local API. Pass the freshly-obtained token
  // directly in the request so we don't rely on localStorage being set yet.
  const authHeader = data.access && (data.access.startsWith("Bearer ") ? data.access : `Bearer ${data.access}`);
  const userRes = await api.get(`/users/${userId}/`, { headers: { Authorization: authHeader } });
