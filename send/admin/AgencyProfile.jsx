...
const AgencyProfile = () => {
  ...
  const [newAgencyForm, setNewAgencyForm] = useState({
    ...
    logo: null,
    user: [] // selected user IDs for the agency (backend requires non-empty list)
  });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch all users for selection in Add Agency form
        try {
          const usersData = await api.get("/users/").then(r => r.data);
          if (!mounted) return;
          setAvailableUsers(Array.isArray(usersData) ? usersData : usersData.results || []);
        } catch (uErr) {
          // If users endpoint not available, just continue without users
          console.warn("Failed to fetch available users:", uErr.message);
        }
      } catch (err) {
        if (!mounted) return;
  ...
  const handleAddAgency = async () => {
    ...
    if (!newAgencyForm.user || newAgencyForm.user.length === 0) {
      showAlert("warning", "Please assign at least one user to the agency");
      return;
    }
    ...
    try {
      ...
      // Append users as repeated form fields so DRF accepts PK list in multipart
      if (newAgencyForm.user && Array.isArray(newAgencyForm.user) && newAgencyForm.user.length > 0) {
        // Some backends accept repeated 'user' fields or 'user[]'. We'll append repeated 'user' entries.
        newAgencyForm.user.forEach((uid) => {
              ...
              <Col md={12}>
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    as="select"
                    multiple
                    value={newAgencyForm.user.map(u => String(u))}
                    onChange={(e) => {
                      const selected = Array.from(e.target.selectedOptions).map(o => parseInt(o.value, 10));
                      setNewAgencyForm({ ...newAgencyForm, user: selected });
                    }}
                    required
                  ...
                  >
                    {availableUsers.map((u) => (
                      <option key={u.id} value={u.id}>
                        {u.first_name || u.username} {u.email ? `(${u.email})` : ''}
                      </option>
                    ))}
                  </Form.Control>
                  <Form.Text className="text-muted">Select one or more users to assign to this agency (required)</Form.Text>
                </Form.Group>
              </Col>
            </Row>
