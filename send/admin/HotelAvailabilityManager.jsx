...
const HotelAvailabilityManager = () => {
  ...
  const [hotelForm, setHotelForm] = useState({
    ...
    category: "ECO",
    distance: "",
    // walking time in minutes (numeric)
  ...
  const roomTypes = [
    { value: "sharing", label: "Sharing", capacity: 1, icon: "👥" },
  ...
  // Render price sections grouped by date range
  const renderPriceSections = (prices) => {
    if (!prices || prices.length === 0) return <div className="text-muted">No price information available.</div>;
    // Group by start/end date
    const groups = {};
    prices.forEach(p => {
      const k = `${p.start_date || ''}|${p.end_date || ''}`;
      if (!groups[k]) groups[k] = { start_date: p.start_date || null, end_date: p.end_date || null, types: {} };
      // normalize room_type key
      const rt = (p.room_type || '').toString();
      groups[k].types[rt] = p.price != null ? p.price : '';
    });

    const orderedTypes = ['sharing', 'double', 'triple', 'quad', 'quint'];
    ...
    return (
      <div>
        <Table striped bordered size="sm">
          <thead>
            <tr>
              <th style={{ minWidth: 160 }}>Date Range</th>
              {orderedTypes.map(t => <th key={t} className="text-center">{t.charAt(0).toUpperCase() + t.slice(1)}</th>)}
            </tr>
          </thead>
          <tbody>
            {Object.keys(groups).map(k => {
              const g = groups[k];
              const start = g.start_date ? new Date(g.start_date).toLocaleDateString() : '-';
              const end = g.end_date ? new Date(g.end_date).toLocaleDateString() : '-';
              return (
                <tr key={k}>
                  <td>{start} — {end}</td>
                  {orderedTypes.map(t => (
                    <td key={t} className="text-center">{g.types[t] !== undefined && g.types[t] !== '' ? `${g.types[t]}` : '—'}</td>
                  ))}
                </tr>
              );
            })}
  ...
  // Return a compact summary for a hotel's prices for table row display
  const getCompactPrices = (prices) => {
    const out = { dateLabel: '-', sharing: null, double: null, triple: null, quad: null, quint: null };
    if (!prices || prices.length === 0) return out;
    // Prefer the price group that matches today, otherwise the first group
    ...
    // find a group whose date range contains today
    for (const k of Object.keys(groups)) {
      const g = groups[k];
      if (g.start_date && g.end_date) {
        const s = new Date(g.start_date);
        const e = new Date(g.end_date);
        if (s <= today && today <= e) { chosen = g; break; }
      }
    }
    if (!chosen) chosen = groups[Object.keys(groups)[0]];
    if (!chosen) return out;
    out.dateLabel = (chosen.start_date ? new Date(chosen.start_date).toLocaleDateString() : '-') + ' — ' + (chosen.end_date ? new Date(chosen.end_date).toLocaleDateString() : '-');
    out.sharing = chosen.types['sharing'] !== undefined ? chosen.types['sharing'] : (chosen.types['sharing'] === undefined ? null : chosen.types['sharing']);
    out.double = chosen.types['double'] !== undefined ? chosen.types['double'] : null;
    out.triple = chosen.types['triple'] !== undefined ? chosen.types['triple'] : null;
    out.quad = chosen.types['quad'] !== undefined ? chosen.types['quad'] : null;
    out.quint = chosen.types['quint'] !== undefined ? chosen.types['quint'] : null;
    return out;
  };
                  ...
                  <thead style={{ backgroundColor: "#f8f9fa" }}>
                    <tr>
                      ...
                      <th style={{ minWidth: "100px" }}>Category</th>
                          ...
                          <th style={{ minWidth: "120px" }}>Walk (min)</th>
                        <th style={{ minWidth: "160px" }}>Price Dates</th>
                        <th style={{ minWidth: "100px" }} className="text-center">Sharing</th>
                        <th style={{ minWidth: "100px" }} className="text-center">Quint</th>
                        <th style={{ minWidth: "100px" }} className="text-center">Quad</th>
                        <th style={{ minWidth: "100px" }} className="text-center">Triple</th>
                        <th style={{ minWidth: "100px" }} className="text-center">Double</th>
                    ...
                    {loading ? (
                      ...
                      filteredHotels.map(hotel => (
                        <tr key={hotel.id}>
                          ...
                          <td>{(hotel.distance !== undefined && hotel.distance !== null && hotel.distance !== "") ? `${Number(hotel.distance).toFixed(2)} km` : "N/A"}</td>
                          <td>{(hotel.walking_time_minutes !== undefined && hotel.walking_time_minutes !== null && hotel.walking_time_minutes !== "") ? `${hotel.walking_time_minutes} min` : "N/A"}</td>
                          {/* Price compact cells */}
                          {(() => {
                            const cp = getCompactPrices(hotel.prices || hotel.prices_data || []);
                            return (
                              <>
                                <td>{cp.dateLabel}</td>
                                <td className="text-center">{cp.sharing !== null && cp.sharing !== '' ? cp.sharing : '—'}</td>
                                <td className="text-center">{cp.quint !== null && cp.quint !== '' ? cp.quint : '—'}</td>
                                <td className="text-center">{cp.quad !== null && cp.quad !== '' ? cp.quad : '—'}</td>
                                <td className="text-center">{cp.triple !== null && cp.triple !== '' ? cp.triple : '—'}</td>
                                <td className="text-center">{cp.double !== null && cp.double !== '' ? cp.double : '—'}</td>
                              </>
                            );
                          })()}
                          <td>
              ...
              <Tab eventKey="hotel" title="Hotel">
                <Form>
                  ...
                  <Card className="mb-3 p-3">
                    <Card.Title className="mb-2">Basic Information</Card.Title>
                    <Row className="g-3 align-items-end">
                      ...
                      {/* Active checkbox removed - using Status select instead */}
                    </Row>
                  </Card>

                  {/* Location & Contact */}
                  <Card className="mb-3 p-3">
                    <Card.Title className="mb-2">Location & Contact</Card.Title>
                    <Row className="g-3">
                      <Col xs={12}>
                        <Form.Group>
                      ...
                      <Col md={4}>
                        <Form.Group>
                          ...
                          <Form.Control
                            type="number"
                            min="0"
                            value={hotelForm.walking_time_minutes}
                            onChange={(e) => setHotelForm({ ...hotelForm, walking_time_minutes: e.target.value })}
                            placeholder="e.g., 10"
                          />
                        </Form.Group>
                      </Col>
                  ...
                  {/* Availability, Media & Options */}
                  <Card className="mb-3 p-3">
                    <Card.Title className="mb-2">Availability & Media</Card.Title>
                  ...
                  {/* Contact Details */}
                  <Card className="mb-3 p-3">
                    <Card.Title className="mb-2">Contact Details</Card.Title>
                  ...
                  <Card className="mb-3 p-2">
                    ...
                    {priceSections.map((p, idx) => (
                      <div key={idx} className="mb-3 p-2 rounded" style={{ backgroundColor: '#fafafa', border: '1px solid #eee' }}>
                        ...
                        <Row className="g-2 align-items-center px-1">
                        ...
                        {(p.bed_prices || []).map((bp, bidx) => (
                          <Row key={bidx} className="g-2 align-items-center mt-2 px-1">
                            <Col md={4}>
            ...
            <Tabs activeKey={addHotelTab} onSelect={(k) => setAddHotelTab(k)} className="mb-3">
              <Tab eventKey="hotel" title="Hotel">
                <Form>
                  ...
                  {/* Active checkbox removed in favor of Status select above */}
                  ...
                  <div className="mt-3">
                    ...
                    <Row className="mb-3">
                      ...
                      <Col md={4}>
                        ...
                        <p>{(selectedHotel.distance !== undefined && selectedHotel.distance !== null && selectedHotel.distance !== "") ? `${Number(selectedHotel.distance).toFixed(2)} km` : "N/A"}</p>
                      </Col>
                      <Col md={4}>
                        <strong>Walking Time:</strong>
                        <p>{(selectedHotel.walking_time_minutes !== undefined && selectedHotel.walking_time_minutes !== null && selectedHotel.walking_time_minutes !== "") ? `${selectedHotel.walking_time_minutes} min` : "N/A"}</p>
                      </Col>
                    </Row>
                    <Row className="mb-3">
                      <Col md={6}>
                    ...
                    <Row className="mb-3">
                      <Col md={12}>
                        ...
                        <div className="mt-2">{renderPriceSections(selectedHotel.prices)}</div>
                      </Col>
                    </Row>
                  </div>
                </Tab>
