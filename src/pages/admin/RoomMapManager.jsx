...
const RoomMapManager = ({ hotelId, onSaved }) => {
  ...
  // Fetch room map data from API
  const fetchRoomMapData = async () => {
    if (!hotelId) return;
    ...
    try {
      ...
      if (transformedFloors.length > 0) {
        ...
        if (transformedFloors[0].rooms?.length > 0) {
          setSelectedRoom(transformedFloors[0].rooms[0]);
        }
      }
    } catch (error) {
      console.error('Error fetching room map data:', error);
      showAlert("danger", "Failed to load room map data");
      // Fall back to demo data if API fails
      setFloors(demoFloors);
      setSelectedFloor(demoFloors[0]);
      setSelectedRoom(demoFloors[0]?.rooms?.[0] || null);
    } finally {
      setLoading(false);
    }
  };

  // Load room map data on component mount
  React.useEffect(() => {
    fetchRoomMapData();
  }, [hotelId]);
  ...
  const handleSaveFloor = async (e) => {
    ...
    try {
      ...
      const floorPayload = {
        hotel: hotelId,
        floor_no: parseInt(floorForm.floor_no),
        floor_title: floorForm.floor_title || `Floor ${floorForm.floor_no}`,
        floor_map_url: floorForm.floor_map_url || '',
      };
      ...
      // Refresh room map data
      await fetchRoomMapData();

      setFloorForm({ floor_no: "", floor_title: "", floor_map_url: "" });
      setEditingFloor(null);
      setShowFloorModal(false);
    } catch (error) {
      console.error('Error saving floor:', error);
      const errorMsg = error.response?.data?.detail || error.response?.data?.error || 'Error saving floor';
  ...
  const handleSaveRoom = async (e) => {
    ...
    try {
      ...
      const details = bedsData.map((bed, index) => ({
        ...
        bed_type: bed.bed_type || roomForm.room_type || 'single'
      }));
      ...
      const roomPayload = {
        ...
        room_number: roomForm.room_no,
        total_beds: parseInt(roomForm.capacity) || 1,
        details: details.length > 0 ? details : [{ bed_number: 1, bed_type: roomForm.room_type || 'single' }]
      };
      ...
      if (editingRoom && editingRoom.id) {
        ...
        showAlert("success", `Room ${roomForm.room_no} updated successfully!`);
      } else {
        // Create new room
        await axios.post(
        ...
        showAlert("success", `Room ${roomForm.room_no} created successfully!`);
      }
      ...
      const errorMsg = error.response?.data?.detail || error.response?.data?.error || 'Error saving room';
      showAlert("danger", errorMsg);
    } finally {
      setLoading(false);
    }
