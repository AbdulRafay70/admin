  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const bookingResponse = await axios.get(`https://b2bapi.saer.pk/api/bookings/?organization_id=${organizationId}&booking_number=${orderNo}`, {
            headers: {
        ...
        if (!booking) {
          try {
            const publicResponse = await axios.get(`https://b2bapi.saer.pk/api/admin/public-bookings/?organization=${organizationId}&booking_number=${orderNo}`, {
              headers: {
                Authorization: `Bearer ${token}`,
                "Content-Type": "application/json",
              }
