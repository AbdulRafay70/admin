...
import { getUniversalList, registerUniversal, getParentOptions } from "../../utils/Api";
import {
...
const UniversalRegister = () => {
  ...
  const loadParentOptions = async () => {
    ...
    try {
      // Use new API endpoint to get real organizations and branches
      const response = await getParentOptions(formData.type);
      const payload = response?.data;
      let items = payload?.data || [];

      // Set the parents from real database data
      setParents(items);
    } catch (error) {
                    ...
                    <div className="d-flex align-items-center mb-4">
                      ...
                      <div>
                        <h5 className="mb-0" style={{ fontWeight: 600, color: "#2c3e50" }}>
                          {formData.type === "agent" ? "Agency" : formData.type.charAt(0).toUpperCase() + formData.type.slice(1)} Registration Form
                        </h5>
                          ...
                          <Col md={6} className="mb-3">
                            <Form.Group>
                              <Form.Label style={{ fontWeight: 500 }}>
                                ...
                                Parent {formData.type === "branch" ? "Organization" : "Branch"} *
                              </Form.Label>
                              <Form.Select
                                name="parent_id"
                            ...
                            >
                              {loading ? (
                                ...
                                <>
                                  ...
                                  Register {formData.type === "agent" ? "Agency" : formData.type.charAt(0).toUpperCase() + formData.type.slice(1)}
                                </>
                              )}
                            </Button>
                            <Button
