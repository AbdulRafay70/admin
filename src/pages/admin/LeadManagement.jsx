...
const LeadManagement = () => {
  ...
  // determine user's branch id (from localStorage 'loginBranch' which may be id or object)
  const getUserBranchId = () => {
    const raw = localStorage.getItem('loginBranch');
    if (!raw) return demoBranches[0].id;
    try {
      const parsed = JSON.parse(raw);
      if (parsed && parsed.id) return parsed.id;
      if (typeof parsed === 'number') return parsed;
      if (typeof parsed === 'string' && parsed.match(/^\d+$/)) return Number(parsed);
    } catch (e) {
      if (raw && raw.match && raw.match(/^\d+$/)) return Number(raw);
    }
    return demoBranches[0].id;
  };
  ...
  // Demo loans data (separate dataset for the Loans tab)
  const demoLoans = [
    {
      id: 1,
      customer_full_name: "Ahmed Ali",
  ...
  // Tasks state (derived from leads follow-ups)
  ...
  const handleAddLoan = () => {
    ...
    if (!newLoanForm.amount) errors.amount = true;
    if (!newLoanForm.loan_promise_date) errors.loan_promise_date = true;
    ...
    const branchObj = demoBranches.find(b => b.id === branchId) || demoBranches[0];
    const newLoan = {
      id: Date.now(),
      customer_full_name: newLoanForm.customer_full_name,
      contact_number: newLoanForm.contact_number,
      cnic_number: newLoanForm.cnic_number,
      email: "",
      branch_name: branchObj.name,
      amount: Number(newLoanForm.amount),
      due_date: newLoanForm.loan_promise_date,
      reason: newLoanForm.reason,
      status: newLoanForm.loan_status,
      recovered_amount: 0,
      recovery_date: null,
    ...
    // also add a follow-up task
    setTasks(prev => [
      {
        id: newLoan.id,
        customer_full_name: newLoan.customer_full_name,
        contact_number: newLoan.contact_number,
        due_date: newLoan.due_date,
        time: "",
        remarks: newLoan.reason,
        status: new Date(newLoan.due_date) < new Date() ? 'overdue' : 'pending'
      },
      ...prev
    ]);

    showAlert("success", "Loan lead added");
    setNewLoanForm({ customer_full_name: "", contact_number: "", cnic_number: "", amount: "", reason: "", loan_promise_date: "", loan_status: "pending", last_contacted_date: "" });
    setActiveTab("loans");
  };
  ...
  useEffect(() => {
    fetchBranches();
    fetchLeads();
    // initialize tasks from demo leads initially
    setTasks(demoLeads
      .filter(l => l.next_followup_date)
      .map(l => ({
        id: l.id,
        customer_full_name: l.customer_full_name,
        contact_number: l.contact_number,
        due_date: l.next_followup_date,
        time: l.next_followup_time,
        remarks: l.remarks,
        status: new Date(l.next_followup_date) < new Date() ? 'overdue' : 'pending'
      }))
    );
  }, []);
  ...
  const handleAddLead = async () => {
    // Validate required fields (compact add should show errors inline)
    const errors = {};
  ...
  const handleEditLead = async () => {
    // validate required followup fields before submitting edit
        ...
        <div className="page-inner" style={{ margin: 0, padding: 0 }}>
          <Container fluid className=" ">
            <div className="page-hero">
              ...
              <p className="page-subtitle">Overview of leads and quick follow-up actions — focus on today's priorities and convert faster.</p>
            </div>
            {/* Alert */}
            {alert.show && (
              ...
              <Col xs={12} sm={6} lg={3} className="mb-3">
                <Card className="stat-card h-100">
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        <p className="text-muted mb-1">LOst lead</p>
                        <h3 className="mb-0">{stats.conversionRate}%</h3>
                      </div>
                      <TrendingUp size={40} className="text-info opacity-50" />
                    </div>
            ...
            <Card className="border-0 shadow-sm">
              ...
              <Card.Body className="p-0">
                {/* Filters and content tabs */}
                <Tabs defaultActiveKey="leads" id="lead-management-tabs" className="mb-3">
