...
const LeadManagement = () => {
  ...
  const submitClearLoan = async () => {
    ...
    try {
      ...
      await axios.put(`http://127.0.0.1:8000/api/leads/update/${loan.id}/`, payload, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const submitCloseLead = async () => {
    ...
    try {
      const updatePayload = { organization: organizationId, branch: selectedLead.branch || selectedLead.branch_id || (userBranchId || getUserBranchId()) };
      ...
      await axios.put(`http://127.0.0.1:8000/api/leads/update/${selectedLead.id}/`, updatePayload, { headers: { Authorization: `Bearer ${token}` } });
                        ...
                        <tbody>
                          {leads.filter(l => {
                            try {
                              ...
                              const nd = normalizeDateYMD(l.next_followup_date);
                              if (!nd) return false;
                              const today = new Date();
                              const tstr = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,'0')}-${String(today.getDate()).padStart(2,'0')}`;
                              if (nd >= tstr) return false; // not overdue
                              if (!l.lead_status) return true;
                              return !(['lost','confirmed'].includes(l.lead_status));
                            } catch (e) { return false; }
                          }).map(lead => {
                            const nd = normalizeDateYMD(lead.next_followup_date) || '-';
                            const daysOverdue = (() => {
                              try {
                                const a = new Date(normalizeDateYMD(lead.next_followup_date));
                                const b = new Date();
                            ...
                            return (
                              <tr key={lead.id}>
                                <td style={{ cursor: 'pointer' }} onClick={() => openViewModal(lead)}>{lead.customer_full_name}</td>
                                <td>{lead.contact_number || lead.whatsapp_number || '-'}</td>
                                <td>{nd}</td>
                                <td>{daysOverdue}</td>
                                <td>
                                  <Button size="sm" variant="outline-primary" className="me-2" onClick={() => openAddRemarksModal(lead)}>Add Remarks</Button>
                                  <Button size="sm" variant="outline-secondary" className="me-2" onClick={() => openNextFollowupModal(lead)}>Next Follow Up</Button>
                                  <Button size="sm" variant="outline-danger" onClick={() => openCloseLeadModal(lead)}>Close Lead</Button>
                                </td>
                              </tr>
                            );
                          })}
              ...
              <Form>
                <Row className="g-2">
                  ...
                  <Col xs={12} md={6} className="d-flex align-items-center">
                    <Form.Check type="checkbox" id="internal-task" label="Internal Task" checked={isInternalTask} onChange={(e) => setIsInternalTask(e.target.checked)} />
                  </Col>
              ...
              <div>
                ...
                <div className="d-flex gap-2 mb-3">
                  ...
                  { (getLoanAmount(selectedLead) > 0 || getRecoveredAmount(selectedLead) > 0) ? (
                    ...
                    <>
                      <Button variant="outline-danger" size="sm" onClick={() => openCloseLeadModal(selectedLead)}>Close Lead</Button>
                      <Button variant="outline-primary" size="sm" onClick={() => openAddRemarksModal(selectedLead)}>Add Remarks</Button>
                    </>
                  )}
                ...
                <Row className="mb-3">
                  <Col md={6} className="mb-3">
                    ...
                    <p className="fw-500">{selectedLead.created_by_username || selectedLead.created_by || '-'}</p>
                  </Col>
                  <Col md={6} className="mb-3">
                    <p className="text-muted small mb-1">Created At</p>
                    <p className="fw-500">{selectedLead.created_at || selectedLead.created_on || selectedLead.created || '-'}</p>
                  </Col>
                </Row>
              ...
              <Row className="g-2">
                ...
                <Col xs={12} md={6}>
                  <Form.Group>
                    ...
                    <Form.Control type="number" value={newLoanForm.amount} onChange={(e) => setNewLoanForm({ ...newLoanForm, amount: e.target.value })} placeholder="Amount (optional)" />
                  </Form.Group>
                </Col>
