...
const CommissionAssignValues = () => {
  ...
  const loadGroupDetails = async (groupId) => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`https://api.saer.pk/api/commissions/rules/${groupId}/`, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
      const data = res.data || {};
      setFormData({
  ...
  const handleSave = async () => {
    ...
    try {
      await axios.patch(`https://api.saer.pk/api/commissions/rules/${selectedGroupId}/`, payload, {
        headers: token ? { Authorization: `Bearer ${token}` } : {},
      });
